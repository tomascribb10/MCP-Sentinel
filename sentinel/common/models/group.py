...
class HostGroup(Base):
    """
    A logical group of sentinel-targets (e.g. 'prod_web_servers').
    ...
    memberships: Mapped[list["TargetGroupMembership"]] = relationship(
        "TargetGroupMembership", back_populates="group", cascade="all, delete-orphan"
    )
...
class TargetGroupMembership(Base):
    """Association table linking targets to host groups."""

    __tablename__ = "target_group_memberships"
    ...
    target: Mapped["Target"] = relationship("Target", back_populates="group_memberships")  # noqa: F821
